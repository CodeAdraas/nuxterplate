import tempus from '../tempes'

export type GenericCallback = (...args: any[]) => void

export const useTempus = (callback: Function, priority = 0) => {
    if (!tempus) return (() => {})
    return tempus.add(callback, priority)
}

export const useRemoveTempus = (callback: Function) => {
    if (!tempus) return
    tempus.remove(callback)
}

export const useThrottle = (callback: GenericCallback, wait: number): GenericCallback => {
